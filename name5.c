...
tName *createName(char *name, char sex, int freq) {
    ...
    // Allocate memory for the 'name' field and copy the provided name
    newName->name = strdup(name);

    if (newName->name == NULL) {
        // Memory allocation for the name field failed, so free the 'newName' structure and return NULL
        free(newName);
        return NULL;
    }
...
void destroyName(void *pName) {
    if (pName != NULL) {
        ...
        // Free the 'tName' structure itself
        free(namePtr);
    }
}